...
def collide_with_walls(sprite, group, dir):
    ...
    if dir == 'y':
        ...
        # the following two line were indented out of the if hits statments sprite velocity is less than zero
        # which lead the mob_sprite to only move on the x axis
            sprite.vel.y = 0
...
class Mob(pygame.sprite.Sprite):
    ...
    def update(self):
        # it needs to know were the player is to point/look at it
        target_dist = self.target.pos - self.pos
        if target_dist.length_squared() < DETECT_RADIUS**2:
            if random() < 0.002:
                choice(self.game.zombie_moan_sounds).play()
            self.rot = target_dist.angle_to(vec(1, 0))
            self.image = pygame.transform.rotate(self.game.mob_image, self.rot)
            #self.rect = self.image.get_rect()
            self.rect.center = self.pos
    ...
    def draw_health(self):
        if self.health > 60:
            col = GREEN
        elif self.health <= 60 and self.health > 25:
            col = YELLOW
        else:
            col = RED
...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, game, pos, dir, damage):
        ...
        self.vel = dir * WEAPONS[self.game.player.weapon]['bullet_speed'] * uniform(0.9, 1.1)
